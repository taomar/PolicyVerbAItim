...
| `LOCAL` | `true` | Whether this deployment issues and verifies **its own** subscription keys. `true` — no gateway in front, so keys minted here are the credential that actually authenticates a caller, and the Integration admin surface is offered. `false` — API Management sits in front and owns caller subscriptions, so a key minted here would be discarded in transit; the `/api/integration/*` routes then answer `404` and stored keys are not verified. **Defaults to `false` in code**, because a deployment that cannot state its shape gets the closed answer; `.env.example` ships `true` for local development. An unparseable value fails at startup rather than resolving to a silent `true`. |
...
| Policy index builds | `policy_index_builds` — one durable row per build attempt, holding its stages and counters while it runs and its outcome once it ends. Unlike upload and extraction progress this is **not** in memory: the same row is the append-only history a publisher reads afterwards and the cross-process slot that keeps two builds from running at once, and neither may be lost with a process. Read one build at `GET /api/policy-index/builds/{operation_id}`, one project's history at `GET /api/policy-sets/{key}/policy-index/builds`, and the whole estate at `GET /api/policy-index/states` (administrator-only). |
...
**One build at a time, across every replica.** Because the pass above is expensive and rewrites a whole index, exactly one policy index build runs at a time for the entire deployment. The coordination is not a process-local flag — the API runs behind more than one replica, so a flag in one process says nothing about another. It is a UNIQUE constraint on `policy_index_builds.active_slot`: a running build holds a single constant value in that column, a finished build holds NULL, and NULLs do not collide, so the database admits exactly one running row however many processes ask. A build whose process dies has its slot reclaimed once the row has not been written to for longer than the build lease (two hours, far longer than any build, because the row is written at every stage boundary and a live build therefore heartbeats).

What that means for the two callers:

- `POST .../policy-index/rebuild` is refused with **409** while another build is running. The response detail names the operation, project and stage in the way. It is refused rather than queued.
- `POST .../publish` still succeeds. The version is published; the rebuild simply does not happen, the response carries `policy_index_build.deferred: true`, and the attempt is recorded in the build history as `deferred` — never ran, which is a different fact from ran and failed.

**Watching a build.** Both accept an optional `operation_id` query parameter, generated by the client before the request (the response does not exist until the build has finished, which is after the point anybody wanted to watch it). Poll `GET /api/policy-index/builds/{operation_id}` for stages, counters and outcome; the record is a database row, so it answers after a page reload and from a replica other than the one running the build. Omitting `operation_id` changes nothing — the server allocates one and returns it.

**Where to look across the estate.** `GET /api/policy-index/states` (administrator-only) reports every project's recorded state, its health classification and its most recent build in one read. `GET /api/policy-sets/{key}/policy-index/builds` reports one project's attempts, newest first. Neither probes the search service, so both load when the search service is the thing that is down.
...
It returns `state`, `document_count`, `policy_document_count`, `rule_document_count`, `projection_profile`, `manifest_state` and `operation_id`. A successful build ends with `manifest_state: "ready"`; anything else means the project is still unmatchable and the decision endpoint will answer `503 index_projection_unavailable` for it. Re-running the command **is** the recovery — the build is a pure function of the database, so it produces the same document ids and overwrites in place. There is no automatic retry, and no rollback to perform: a failed build never removes the previous documents, it only leaves the manifest short of `ready`. A `409` means another build holds the slot; wait and re-run.
