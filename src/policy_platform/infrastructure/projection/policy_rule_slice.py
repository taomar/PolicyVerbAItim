...
#: One piece of the joined text, and whether this system generated it. The flag
#: is what makes provenance survive the join: after it, a slug of an English
#: phrase and an English phrase are the same shape, and nothing downstream can
#: tell them apart by looking.
_Segment = tuple[str, bool]


def _fact_segments(facts: dict, ref: object) -> list[_Segment]:
    """One fact's matchable pieces: its generated key, the document's phrase, its type.

    ``name`` is a slug this system generated from ``source_phrase``; the phrase
    itself is the document speaking. They sit next to each other here, which is
    the only place that difference is still knowable without guessing at shape.
    """
    ...
    if not isinstance(entry, dict):
        return []
    return [
        (str(entry.get("name") or ""), True),
        (str(entry.get("source_phrase") or ""), False),
        (str(entry.get("data_type") or ""), True),
    ]


def _fact_text(facts: dict, ref: object) -> str:
    """The same pieces, joined — the string this function has always returned."""

    return " ".join(text for text, _ in _fact_segments(facts, ref))
...
def rule_text(rule: dict, *, spans: dict, facts: dict) -> str:
    ...
    return rule_text_parts(rule, spans=spans, facts=facts)[0]


def rule_text_parts(
    rule: dict, *, spans: dict, facts: dict
) -> tuple[str, tuple[tuple[int, int], ...]]:
    """The rule's matchable text, and where in it the machine-made terms sit.

    Returns the same string :func:`rule_text` has always returned — the join
    below is the one thing in here that must not move, because the request side
    scores that exact string and an index built on a different one is an index
    answering a different question.

    THE SECOND VALUE IS POSITIONS, NOT WORDS

    The two kinds of content in this string have opposite requirements
    downstream. The document's own sentences are *prose*: they are meant to be
    rendered into the processing language, and a rendering that reproduced them
    unchanged would have failed. The rest — normalised ids, tags, scope values,
    effect and modality names — are identifiers this system generated by slugging
    a phrase; they are keys, not language, and rendering them is meaningless.

    Nothing here can tell those apart by looking at them: a slug made from an
    English phrase reads as English words joined with hyphens. But nothing has to
    look, because at this point we still know which *fields* they came from.

    What is returned is therefore ``(start, end)`` pairs into the text, not the
    terms themselves. A term is not a location: the same generated value can
    appear twice in one rule and can also occur, by coincidence, inside a
    sentence that merely uses the word — ``action``, ``leave``, ``you``. Handing
    a caller the value would let it find those too, and protecting a stretch of
    prose because it spells a key is the same failure as rendering a key because
    it spells prose. Positions cannot be mistaken that way: each one is the exact
    occurrence this function put there, and no other.

    Spans are returned in ascending order and never overlap.
    """

    parts: list[list[_Segment]] = []

    def prose(value: object) -> list[_Segment]:
        """A piece the document said, to be rendered like any other language."""

        return [(str(value or ""), False)]

    def machine(value: object) -> list[_Segment]:
        """A piece this system generated, to be carried through untouched."""
    ...
    for ref in rule.get("evidence_refs") or []:
        ...
        if isinstance(span, dict) and span.get("text"):
            parts.append(prose(span["text"]))

    effect = rule.get("effect") or {}
    if isinstance(effect, dict):
        # ``action`` reads as a generated field name but holds the document's own
        # predicate — the longest one in this corpus is 378 characters of prose.
        # ``type`` beside it is a closed vocabulary. Shape would have got this
        # backwards; the field profile did not.
        parts.append(prose(effect.get("action")))
        parts.append(machine(effect.get("type")))
    parts.append(machine(rule.get("rule_type")))
    # ``modality`` is likewise the sentence's own wording — "It is the
    # responsibility of", "is entitled to" — and has to be rendered like any
    # other prose.
    parts.append(prose(rule.get("modality")))

    for required in rule.get("required_facts") or []:
        if isinstance(required, dict):
            # ``phrase`` and ``unit`` are said; ``name`` is a slug of the phrase
            # and the rest are closed vocabularies. The join keeps every
            # position, empty or not, because that is what the request side
            # scores.
            parts.append(
                [
                    (str(required.get(key) or ""), key not in ("phrase", "unit"))
        ...
        for group in ("applies", "outcome"):
            for attribute in attributes.get(group) or []:
                if not isinstance(attribute, dict):
                    continue
                parts.append(machine(attribute.get("attribute")))
                if attribute.get("text"):
                    parts.append(prose(attribute["text"]))
                if attribute.get("fact_ref"):
                    parts.append(_fact_segments(facts, attribute["fact_ref"]))

    for fact in rule.get("facts") or []:
        if isinstance(fact, dict):
            parts.append(_fact_segments(facts, fact.get("ref")))
            parts.append([(str(role or ""), True) for role in fact.get("roles") or []])

    for exception in rule.get("exceptions") or []:
        if isinstance(exception, dict):
            parts.append(prose(exception.get("description")))
            # A unit is a measurement word the document used, not a key.
            parts.append(prose(exception.get("limit_unit")))

    for advice in rule.get("advice") or []:
        if isinstance(advice, dict):
            parts.append(prose(advice.get("text")))

    parts.extend(machine(tag) for tag in rule.get("tags") or [])

    scope = rule.get("scope")
    if isinstance(scope, dict):
        for values in scope.values():
            parts.extend(machine(value) for value in values or [])

    return _join_with_spans(parts)


def _join_with_spans(
    parts: Sequence[Sequence[_Segment]],
) -> tuple[str, tuple[tuple[int, int], ...]]:
    """Join the parts the way this module always has, recording where keys landed.

    The string produced here is byte-for-byte the one the request side scores:
    each part is its segments joined by a single space keeping every position,
    empty or not, and the parts themselves are joined by a single space with the
    empty ones dropped. Offsets are accumulated as that happens, which is the
    only moment at which a term's position and its provenance are both known.
    """

    chunks: list[str] = []
    machine_made: list[tuple[int, int]] = []
    cursor = 0
    for segments in parts:
        part = " ".join(text for text, _ in segments)
        if not part:
            continue
        if chunks:
            cursor += 1  # the single space this join is about to insert
        inner = cursor
        for text, is_machine in segments:
            if is_machine and text.strip():
                machine_made.append((inner, inner + len(text)))
            inner += len(text) + 1
        chunks.append(part)
        cursor += len(part)
    return " ".join(chunks), tuple(machine_made)
