...
async def rebuild_policy_index_endpoint(
    ...
    same quota, and two on one project race each other's manifest writes. So a
    request that arrives while another build is running is refused with **409**,
    naming the operation and project that hold the slot. It is refused rather
    than queued: a queue needs a worker to drain it and an answer for what
    happens when the process holding it dies, and saying no is the smaller
    coherent behaviour.

    ``operation_id`` is optional and may be generated by the client before the
    call, exactly as an upload's is, so a page can poll
    ``GET /api/policy-index/builds/{operation_id}`` for the stages while this
    request is still open. Omitting it changes nothing about the build; the
    server allocates one and returns it.
    """
    ...
    if not tracked.accepted:
        # 409, and it names what is running. "Try again later" without saying
        # what is in the way is a message nobody can act on — and the detail
        # carries an operation id, a project key and a stage, which are the
        # three things an operator needs and none of which is source text.
        raise HTTPException(
            status_code=409,
            detail={
                "message": (
                    "another policy index build is already running; only one runs at a "
                    "time because a build re-renders a whole corpus and rewrites an "
                    "entire search index"
                ),
                "active": build_payload(tracked.conflict),
                "operation_id": tracked.build.operation_id if tracked.build else None,
            },
        )

    outcome = tracked.outcome
    assert outcome is not None  # accepted builds always carry one; see the dataclass
    await record_policy_index_build_state(session, policy_set_id=policy_set.id, outcome=outcome)
    await session.commit()
    payload = policy_index_build_outcome_payload(outcome)
    # The handle the client polls on, echoed back so a caller that did not
    # generate one can still find this build in the history it just wrote.
    payload["operation_id"] = tracked.build.operation_id if tracked.build else None
    return payload


@router.get("/{key}/policy-index/builds")
async def list_policy_index_builds_endpoint(
    key: str, session: AsyncSession = Depends(get_session)
) -> dict:
    """This project's index build attempts, newest first, plus what is running now.

    WHY A HISTORY EXISTS AT ALL

    `policy_index_states` holds one row per project and every attempt overwrites
    it, so a publisher whose build failed an hour ago, was retried, and failed
    again sees one row saying `failed` — indistinguishable from a first failure,
    and useless for deciding whether to retry again or to ask for help. The
    attempts are appended to `policy_index_builds` instead, and this reads them.

    `active` is the build holding the single global build slot, whichever project
    it belongs to and whichever replica is running it. It is returned here rather
    than from a separate route because the surface that shows a project's history
    is the same one that offers to rebuild it, and that offer has to be disabled
    while any build is running — including one for a different project.

    Counts, stage keys, timestamps and bounded failure descriptions. No source
    text, no rendered text and no service reply reaches this payload.
    """
    ...
    builds = await builds_for_policy_set(session, policy_set_id=policy_set.id)
    active = await running_build(session)
    return {
        "policy_set_key": key,
        "builds": [build_payload(build) for build in builds],
        # Deliberately the whole payload rather than a boolean: a UI that only
        # knew "something is running" could disable its button but could not say
        # what it is waiting for, and an operator staring at a disabled control
        # with no explanation reads it as a broken page.
        "active": build_payload(active),
    }
